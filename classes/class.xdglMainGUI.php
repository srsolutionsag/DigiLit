...
class xdglMainGUI
{
    ...
    public function executeCommand()
    {
        ...
        $this->tabs->addTab(self::TAB_REQUESTS, $this->pl->txt('tab_' . self::TAB_REQUESTS),
            $this->ctrl->getLinkTarget($xdglRequestGUI));
        ...
        if (ilObjDigiLitAccess::isAdmin()) {
            ...
            $this->tabs->addTab(self::TAB_SETTINGS, $this->pl->txt('tab_' . self::TAB_SETTINGS),
                $this->ctrl->getLinkTarget($xdglConfigGUI));
            if (xdglConfig::getConfigValue(xdglConfig::F_USE_LIBRARIES)) {
                $this->tabs->addTab(self::TAB_LIBRARIES, $this->pl->txt('tab_' . self::TAB_LIBRARIES),
                    $this->ctrl->getLinkTarget($xdglLibraryGUI));
            }
        }
