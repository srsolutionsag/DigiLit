...
class xdglRequestUsageFactory implements xdglRequestUsageFactoryInterface
{
    ...
    public function deleteUsagesAndDigiLitObjectsByRequestId($request_id)
    {
        ...
        $ilDB->manipulate("DELETE FROM " . xdglRequest::TABLE_NAME . " WHERE request_id = " . $ilDB->quote($request_id,
                'integer'));
    }
    ...
    {
        global $ilDB;
        $ilDB->manipulate("DELETE FROM " . xdglRequestUsage::TABLE_NAME . " WHERE obj_id = " . $ilDB->quote($obj_id,
                'integer'));
